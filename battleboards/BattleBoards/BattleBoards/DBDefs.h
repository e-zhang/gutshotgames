//
//  DBDefs.h
//  sumosmash
//
//  Created by Eric Zhang on 4/6/13.
//  Copyright (c) 2013 gutshotgames. All rights reserved.
//

#ifndef DBDEFS_H_
#define DBDEFS_H_

static NSString * const DB_USER_ID = @"playerId";
static NSString * const DB_FB_ID = @"fb_id";
static NSString * const DB_USER_NAME = @"name";
static NSString * const DB_CONNECTED = @"connected";
static NSString * const DB_START_LOC = @"start_location";

static NSString * const DB_PROFILES = @"battleboard_users";
static NSString * const DB_UPDATES = @"battleboard_gameinvites";
static NSString * const DB_GAMES = @"battleboard_games";

static NSString * const DB_MOVES = @"player_moves";
static NSString * const DB_BOMBS = @"player_bombs";

//static NSString * const DB_CHAT = @"chat";

static NSString * const DB_TEAM_INVITES = @"team invites";

#endif